<template>
  <div class="cart-item">
    <div class="cart-item__left">
      <img
        :src="cart_item_data.image"
        alt="Фото товара"
        class="cart-item__left-image"
      />
      <div class="cart-item__left-descr">
        <span class="cart-item__left-descr__title">{{
          cart_item_data.title
        }}</span>
        <span class="cart-item__left-descr__price"
          >{{ cart_item_data.price }}₽</span
        >
      </div>
    </div>
    <div class="cart-item__right">
      <div class="cart-item__middle">
        <button class="cart-item__middle-btn btn-red" @click="decrementItem">
          <span>-</span>
        </button>
        <div class="count-products">{{ cart_item_data.quantity }}</div>
        <button class="cart-item__middle-btn btn-green" @click="incrementItem">
          <span>+</span>
        </button>
      </div>
      <div class="cart-item__right">
        <button @click="deleteItemCart" class="button-delete">
          <svg
            width="18"
            height="20"
            viewBox="0 0 18 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M15.8789 6.71882L15.9784 6.72017C16.3475 6.75069 16.6304 7.05716 16.65 7.42605L16.6405 7.63174L16.326 11.483L15.9961 15.2414C15.9263 15.9917 15.8638 16.6245 15.8099 17.1227C15.6225 18.8588 14.4955 19.9323 12.7966 19.9641C10.1494 20.013 7.60477 20.0125 5.13373 19.9591C3.48398 19.9244 2.37366 18.8393 2.18955 17.1297L2.0623 15.8702L1.83994 13.427L1.61216 10.7461L1.35172 7.52788C1.31935 7.11498 1.61951 6.75335 2.02215 6.72016C2.39123 6.68973 2.7183 6.94584 2.79519 7.30677L2.82511 7.60173L3.06966 10.6187L3.33669 13.7459C3.45646 15.0996 3.56034 16.1952 3.64346 16.9648C3.74838 17.939 4.26138 18.4404 5.16411 18.4593C7.61585 18.5124 10.1415 18.5129 12.7701 18.4643C13.7277 18.4464 14.2488 17.9499 14.356 16.9574L14.4827 15.7046C14.5198 15.3185 14.5594 14.8923 14.6013 14.4293L14.8686 11.3538L15.1906 7.4075C15.2204 7.02902 15.5192 6.7389 15.8789 6.71882ZM0.73139 4.98918C0.327455 4.98918 0 4.65338 0 4.23916C0 3.85945 0.275153 3.54564 0.632145 3.49597L0.73139 3.48913H3.91772C4.29636 3.48913 4.62785 3.23928 4.74642 2.87929L4.77543 2.76813L5.02304 1.50533C5.24111 0.668966 5.9492 0.0734931 6.779 0.00633149L6.93592 0H11.0639C11.9075 0 12.6523 0.546275 12.9391 1.39039L12.9874 1.55209L13.2243 2.76783C13.2987 3.14872 13.6025 3.4332 13.9701 3.48177L14.0821 3.48913H17.2686C17.6725 3.48913 18 3.82493 18 4.23916C18 4.61887 17.7248 4.93267 17.3679 4.98234L17.2686 4.98918H0.73139ZM11.0639 1.50006H6.93592C6.7307 1.50006 6.54829 1.62322 6.47252 1.77803L6.44682 1.84604L6.20979 3.06238C6.1808 3.21084 6.13879 3.3538 6.08512 3.49012L11.9148 3.49031C11.8813 3.40526 11.8523 3.31763 11.8282 3.22768L11.79 3.06208L11.5636 1.8928C11.5107 1.68991 11.3473 1.54138 11.1502 1.50742L11.0639 1.50006Z"
              fill="#d83636"
            />
          </svg>
        </button>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "app-item",
  data: () => ({}),
  props: {
    cart_item_data: {
      type: Object,
      default() {
        return {};
      },
    },
  },
  methods: {
    decrementItem() {
      this.$emit("decrementCart");
    },
    incrementItem() {
      this.$emit("incrementCart");
    },
    deleteItemCart() {
      this.$emit("deleteItemCart");
    },
  },
  mounted() {},
};
</script>
